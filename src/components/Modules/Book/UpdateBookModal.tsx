...
import type { IBook, IImageUpload } from "@/types";
...
type Props = { bookInfo: Partial<IBook> };
...
export const UpdateBookModal: React.FC<Props> = ({ bookInfo }) => {
  // const { data: bookInfo, isLoading: bookInfoLOading } = useGetSingleBooksQuery(
  //   _id,
  //   {
  //     skip: !_id,
  //   }
  ...
  const form = useForm<BookFormValues>({
    ...
    defaultValues: {
      ...
      bookType: bookInfo?.bookType || "OTHERS",
      language: bookInfo?.language || "en",
      status: bookInfo?.status || "COMPLETE",
      content: bookInfo?.content || [{ chapter: "", story: "" }],
    },
  });
  ...
  const onSubmit = async (values: BookFormValues) => {
    ...
    try {
      await update({ _id: bookInfo._id!, formData }).unwrap();
      // console.log(result);
